...
def ask(payload: AskIn):
    ...
    audit_ctx = "\n".join(
    f"- [AUDIT {h['file']} — {h['section']} — {h['clause']}] {h['question']}" for h in audit_top) or "No audit checklist matches."
    ims_ctx = "\n".join(
    f"- [IMS {h['relpath']}] {h['chunk'][:600]}" for h in ims_top) or "No IMS excerpts found."
